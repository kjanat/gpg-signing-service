...
export const ArmoredPrivateKeySchema = z
	...
	.refine(
		(val) => {
			...
			const hasBase64 = lines.some((line) => /^[A-Za-z0-9+/=]{1,76}$/.test(line.trim()));
			if (!hasBase64) return false;

			// Should contain checksum line (starts with '=')
			const hasChecksum = lines.some((line) => /^=[A-Za-z0-9+/=]{4}$/.test(line.trim()));

			return hasChecksum;
		},
		{
			message: "Invalid PGP armored format - must include base64 data and checksum",
		},
	)
 ...
 */
export const KeyListResponseSchema = z.object({ keys: z.array(KeyListItemSchema) }).openapi("KeyListResponse");
