...
import { AppError, errorResponse, handleUnknownError, isAppError } from "#utils/errors";
...
describe("errors.ts - Error handling utilities", () => {
	...
	describe("errorResponse()", () => {
		...
		it("should use default status (500) when not provided", async () => {
			...
			expect(c.json).toHaveBeenCalledWith(expect.any(Object), HTTP.InternalServerError);
		});
		...
		it("should handle context with nested objects", () => {
			...
			expect(logger.error).toHaveBeenCalledWith("Error", undefined, expect.objectContaining(context));
		});
	...
	describe("handleUnknownError()", () => {
		...
		it("should return Response from errorResponse", () => {
			...
			const response = handleUnknownError(c, error, "fallback", "INTERNAL_ERROR");
	...
	describe("AppError class", () => {
		...
		it("should accept context object", () => {
			...
			const error = new AppError("Error with context", "AUDIT_ERROR", HTTP.InternalServerError, context);
		...
		it("should have status property defined", () => {
			const error = new AppError("Test", "SIGN_ERROR", HTTP.InternalServerError);
		...
		it("should have context property defined", () => {
			...
			const error = new AppError("Test", "SIGN_ERROR", HTTP.InternalServerError, context);
	...
	describe("Integration scenarios", () => {
		it("should handle AppError in handleUnknownError", () => {
			...
			const appError = new AppError("App error message", "KEY_NOT_FOUND", HTTP.NotFound);
		...
		it("should create AppError and use with errorResponse", () => {
			...
			const appError = new AppError("Processing failed", "KEY_PROCESSING_ERROR", HTTP.BadRequest);
		...
		it("should chain error handling with context preservation", () => {
			...
			const error = new AppError("Signing failed", "SIGN_ERROR", HTTP.BadRequest, context);
