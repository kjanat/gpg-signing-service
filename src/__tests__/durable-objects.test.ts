...
import { fetchKeyStorage, fetchRateLimiter, parseDOResponse } from "#utils/durable-objects";
...
describe("Durable Object Helpers", () => {
	...
	describe("fetchKeyStorage - Error Cases", () => {
		it("should propagate fetch errors", async () => {
			...
			await expect(fetchKeyStorage(env, "/test")).rejects.toThrow("Fetch failed");
		});
	...
	describe("fetchRateLimiter - Happy Path", () => {
		it("should fetch from RATE_LIMITER with identity", async () => {
			const mockResponse = new Response(JSON.stringify({ allowed: true, remaining: 10 }));
			const mockFetch = vi.fn().mockResolvedValue(mockResponse);
			const env = {
				RATE_LIMITER: {
	...
	describe("fetchRateLimiter - Error Cases", () => {
		it("should propagate DO errors", async () => {
			...
			await expect(fetchRateLimiter(env, "test")).rejects.toThrow("DO unavailable");
		});
	...
	describe("parseDOResponse - Error Cases", () => {
		it("should throw on 404 with error message", async () => {
			...
			await expect(parseDOResponse(response, (d) => d)).rejects.toThrow("Not found");
		});

		it("should throw on 500 with generic message", async () => {
			const response = new Response(JSON.stringify({}), { status: 500 });

			await expect(parseDOResponse(response, (d) => d)).rejects.toThrow("DO request failed: 500");
		});
		...
		it("should throw on 503 Service Unavailable", async () => {
			...
			await expect(parseDOResponse(response, (d) => d)).rejects.toThrow("Service down");
		});
		...
		it("should throw on 400 Bad Request", async () => {
			...
			await expect(parseDOResponse(response, (d) => d)).rejects.toThrow("Bad input");
		});
		...
		it("should propagate parser errors", async () => {
			...
			await expect(parseDOResponse(response, strictParser)).rejects.toThrow("Parser validation failed");
		});
	...
	describe("parseDOResponse - Edge Cases", () => {
		...
		it("should handle 401 Unauthorized", async () => {
			...
			await expect(parseDOResponse(response, (d) => d)).rejects.toThrow("Unauthorized");
		});
		...
		it("should handle 429 Too Many Requests", async () => {
			...
			await expect(parseDOResponse(response, (d) => d)).rejects.toThrow("Rate limited");
		});

		it("should handle error field as object instead of string", async () => {
			const response = new Response(JSON.stringify({ error: { code: 500, msg: "Error" } }), { status: 500 });

			// When error is object, toString() gives "[object Object]"
			await expect(parseDOResponse(response, (d) => d)).rejects.toThrow("[object Object]");
		});
		...
		it("should use parser with type guards", async () => {
			const response = new Response(JSON.stringify({ id: "123", active: true }), { status: 200 });
		...
		it("should fail parser type guards", async () => {
			...
			await expect(parseDOResponse(response, parser)).rejects.toThrow("id must be string");
		});
	});
	...
	describe("Integration Scenarios", () => {
		it("should handle key retrieval workflow", async () => {
			...
			const mockFetch = vi.fn().mockResolvedValue(new Response(JSON.stringify(keyData)));
		...
		it("should handle key not found workflow", async () => {
			...
			await expect(parseDOResponse(response, (d) => d)).rejects.toThrow("Key not found");
		});

		it("should handle rate limit check workflow", async () => {
			const limitData = { allowed: false, remaining: 0, resetAt: 1234567890 };

			const mockFetch = vi.fn().mockResolvedValue(new Response(JSON.stringify(limitData)));
			...
			const response = await fetchRateLimiter(env, "https://github.com:repo:org/name");
