...
describe("getAuditLogs", () => {
	it("should return audit logs with default options", async () => {
		...
		expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("SELECT * FROM audit_logs"));
	...
	it("should apply action filter", async () => {
		...
		expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("AND action = ?"));
	...
	it("should apply subject filter with LIKE and ESCAPE clause", async () => {
		...
		expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("AND subject LIKE ? ESCAPE '\\'"));
	...
	it("should escape % wildcard in subject filter to prevent pattern injection", async () => {
		...
		expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("ESCAPE '\\'"));
	...
	it("should handle combined SQL injection attempt in subject", async () => {
		...
		expect(db._mockBind).toHaveBeenCalledWith("%\\%'; DROP TABLE audit\\_logs; --%", 100, 0);
	});
	...
	it("should apply date range filters", async () => {
		...
		expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("AND timestamp >= ?"));
		expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("AND timestamp <= ?"));
