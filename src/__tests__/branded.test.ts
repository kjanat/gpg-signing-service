...
describe("Branded Types", () => {
	describe("createKeyId", () => {
		...
		it("should reject KeyId shorter than 16 characters", () => {
			expect(() => createKeyId("A1B2C3D4E5F6071")).toThrow("Invalid KeyId format: A1B2C3D4E5F6071");
		});

		it("should reject KeyId longer than 16 characters", () => {
			expect(() => createKeyId("A1B2C3D4E5F607189")).toThrow("Invalid KeyId format: A1B2C3D4E5F607189");
		});
		...
		it("should reject non-hex characters (G-Z)", () => {
			expect(() => createKeyId("G1B2C3D4E5F60718")).toThrow("Invalid KeyId format: G1B2C3D4E5F60718");
		});
	...
	describe("createKeyFingerprint", () => {
		...
		it("should create valid KeyFingerprint from uppercase hex string", () => {
			const result = createKeyFingerprint("A1B2C3D4E5F607189A1B2C3D4E5F607189A1B2C3");
		...
		it("should handle mixed case input", () => {
			const result = createKeyFingerprint("AbCdEf0123456789AbCdEf0123456789AbCdEf01");
			expect(result).toBe("ABCDEF0123456789ABCDEF0123456789ABCDEF01");
		});

		it("should accept valid 40-character hex string", () => {
			const result = createKeyFingerprint("0123456789ABCDEF0123456789ABCDEF01234567");
		...
		it("should reject KeyFingerprint shorter than 40 characters", () => {
			expect(() => createKeyFingerprint("A1B2C3D4E5F607189A1B2C3D4E5F6071")).toThrow("Invalid KeyFingerprint format:");
		});

		it("should reject KeyFingerprint longer than 40 characters", () => {
			expect(() => createKeyFingerprint("A1B2C3D4E5F607189A1B2C3D4E5F607189A1B2C3D4")).toThrow(
				"Invalid KeyFingerprint format:",
			);
		});

		it("should reject empty string", () => {
			expect(() => createKeyFingerprint("")).toThrow("Invalid KeyFingerprint format:");
		});

		it("should reject non-hex characters (G-Z)", () => {
			expect(() => createKeyFingerprint("G1B2C3D4E5F607189A1B2C3D4E5F607189A1B2C3")).toThrow(
				"Invalid KeyFingerprint format:",
			);
		});

		it("should reject special characters", () => {
			expect(() => createKeyFingerprint("A1B2-C3D4-E5F6-0718-9A1B-2C3D-4E5F-6071-89A1")).toThrow(
				"Invalid KeyFingerprint format:",
			);
		});

		it("should reject whitespace", () => {
			expect(() => createKeyFingerprint("A1B2C3D4E5F60718 A1B2C3D4E5F607189A1B2C3")).toThrow(
				"Invalid KeyFingerprint format:",
			);
		});
	});
...
-----END PGP PRIVATE KEY BLOCK-----`;
		...
		it("should reject completely empty key", () => {
			expect(() => createArmoredPrivateKey("")).toThrow("Invalid ArmoredPrivateKey: missing PGP header");
		});
...
-----END PGP PRIVATE KEY BLOCK-----`;
	...
	describe("createIdentity", () => {
		...
		it("should not validate format, just concatenate", () => {
			...
			const result = createIdentity("invalid issuer format", "invalid:subject:format");
		...
		it("should handle whitespace in identifiers", () => {
			const result = createIdentity("issuer with spaces", "subject with spaces");
	...
	describe("Type Branding", () => {
		...
		it("should maintain type distinction for KeyFingerprint", () => {
			const fingerprint = createKeyFingerprint("A1B2C3D4E5F607189A1B2C3D4E5F607189A1B2C3");
...
-----END PGP PRIVATE KEY BLOCK-----`;
	...
	describe("Edge Cases and Boundary Conditions", () => {
		...
		it("should handle KeyFingerprint with all zeros", () => {
			const result = createKeyFingerprint("0000000000000000000000000000000000000000");
			expect(result).toBe("0000000000000000000000000000000000000000");
		});

		it("should handle KeyFingerprint with all Fs", () => {
			const result = createKeyFingerprint("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
