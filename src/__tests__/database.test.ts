...
describe("D1QueryBuilder", () => {
	...
	describe("where clause", () => {
		it("should add single where condition", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").where("action", "sign");
		...
		it("should support string values in where", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").where("issuer", "https://github.com");
			const { query, params } = builder.build();
		...
		it("should support numeric values in where", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").where("id", 123);
			const { query, params } = builder.build();
		...
		it("should add three or more where conditions", () => {
			...
			expect(query).toContain("WHERE action = ? AND issuer = ? AND success = ?");
	...
	describe("whereBetween clause", () => {
		it("should add BETWEEN condition", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").whereBetween(
				"timestamp",
				"2024-01-01",
				"2024-12-31",
			);
	...
	describe("orderBy clause", () => {
		it("should add ORDER BY DESC by default", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").orderBy("timestamp");
			const { query } = builder.build();
		...
		it("should add ORDER BY with specified direction (ASC)", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").orderBy("timestamp", "ASC");
			const { query } = builder.build();
		...
		it("should add ORDER BY with specified direction (DESC)", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").orderBy("timestamp", "DESC");
			const { query } = builder.build();
		...
		it("should support multiple orderBy (last one wins)", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").orderBy("timestamp", "DESC").orderBy("id", "ASC");
			const { query } = builder.build();
		...
		it("should place ORDER BY before LIMIT", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").orderBy("timestamp", "DESC").limit(10);
			const { query } = builder.build();
	...
	describe("limit clause", () => {
		...
		it("should add LIMIT with specified offset", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").limit(20, 5);
			const { query, params } = builder.build();
		...
		it("should support pagination", () => {
			const builder = new D1QueryBuilder("SELECT * FROM audit_logs").limit(100, 200);
	...
	describe("execute method", () => {
		...
		it("should pass all params to bind", async () => {
			...
			expect(mockBind).toHaveBeenCalledWith("sign", "https://github.com", 50, 10);
		});
		...
		it("should propagate database errors", async () => {
			...
			await expect(builder.execute(mockDb)).rejects.toThrow("Database connection failed");
		});
		...
		it("should handle transformer errors", async () => {
			...
			await expect(builder.execute(mockDb, transformAuditLogRow)).rejects.toThrow();
		});
