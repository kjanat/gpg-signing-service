 ...
 */
export async function verifyServiceToken(db: D1Database, token: string): Promise<ServiceTokenPolicy | null> {
	const row = await db
		.prepare(
...
> {
	const { results } = await db
       ...
       FROM service_tokens ORDER BY created_at DESC`,
		...
		.all<ServiceTokenRow & { created_at: string; last_used_at: string | null }>();
...
export async function revokeServiceToken(db: D1Database, id: string): Promise<boolean> {
