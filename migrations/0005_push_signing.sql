-- The GitHub App's push-signing handler records what it did, so a branch this
-- service rewrote is reconstructable from the audit trail rather than only from
-- the repository it rewrote.
--
-- SQLite cannot alter a CHECK constraint, so the table is rebuilt in place —
-- the same pattern as 0002 and 0003.
