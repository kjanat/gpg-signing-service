...
Decision 1 is carried out by `scripts/key-handoff.sh publish-revocation`, which
imports the existing offline certificate, proves the key revoked locally, then
publishes and independently fetches back from each target. It cannot generate a
certificate and refuses to run in CI. See the
[key handoff runbook](../key-handoff-runbook.md).
...
- [Key handoff runbook](../key-handoff-runbook.md) — the operator procedure
