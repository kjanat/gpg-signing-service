...
describe("Admin Auth - Timing Attack Resistance", () => {
	...
	it("should take same time for wrong-length tokens", async () => {
		...
		const variance = [shortTime, longTime, sameLengthTime].map(
			(t) => Math.abs(t - avgTime) / avgTime,
		);
			...
			mockDb.prepare = vi.fn().mockReturnValue({
				bind: vi.fn().mockReturnValue({
					all: vi.fn().mockResolvedValue({
						results: [{ id: "1", timestamp: "2025-01-01T00:00:00Z" /* ... */ }],
					}),
				}),
			});
...
describe("Error Handling Utilities", () => {
	...
	describe("handleUnknownError", () => {
		...
		it("should log with UNKNOWN_ERROR code", () => {
			...
			handleUnknownError(c, new Error("test"), "Fallback", "UNKNOWN_ERROR");
	...
	describe("AppError class", () => {
		it("should create error with required fields", () => {
			const error = new AppError("Test error", "TEST_CODE", HTTP.BadRequest);
