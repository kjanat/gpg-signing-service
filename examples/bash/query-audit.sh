...
format_logs() {
	jq -r '.logs[] | "\(.timestamp | split("T")[0]) \(.timestamp | split("T")[1]) | \(.action) | \(.subject) | \(if .success then "✓" else "✗" end)"'
}
...
case "${1:-all}" in
	...
	signing)
		...
		query_logs "action=sign&limit=100"
		format_logs
		;;

	keys)
		echo "Key management operations:"
		query_logs "action=key_upload,key_rotate&limit=100"
		format_logs
		;;
	...
	by-repo)
		...
		query_logs "subject=${repo}&limit=100"
		format_logs
		;;
